...
def uniform_cost_search(graph, start, goal_states):
    ...
    while priority_queue:
        ...
        if current_node in goal_states:
            goal_states.remove(current_node)
            # Reached a goal state, check if all goal states have been visited
            if not goal_states:
                # Construct the path for each goal state
                paths = []
                for goal in parent:
                    path = []
                    node = goal
...
paths = uniform_cost_search(graph, initial_state, goal_states)
