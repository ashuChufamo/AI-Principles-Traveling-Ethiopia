...
def uniform_cost_search(graph, start, goals):
    ...
    while priority_queue:
        ...
        if current_node in goals:
            # Reached a goal state, construct the path
            path = []
...
for goal_node in goal_states:
    goal_path = uniform_cost_search(graph, current_node, [goal_node])
    if goal_path:
        path.extend(goal_path[:-1])  # Exclude the last node to avoid duplication
        current_node = goal_node
    else:
        print("No path found from", current_node, "to", goal_node)
        break
else:
    # All goal states reached, include the last node
    path.append(goal_states[-1])
